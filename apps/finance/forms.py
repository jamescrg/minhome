    ...
    def clean_symbol(self):
        ...
        if not self._validate_symbol_with_api(symbol):
            raise ValidationError(
                f"Symbol '{symbol}' not found in CoinMarketCap. Please check the symbol and try again."
            )
...
class SecuritiesSymbolForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "name": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "e.g., Tesla"}
            ),
            "exchange": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "e.g., NYSE, NASDAQ"}
            ),
            "is_active": forms.CheckboxInput(attrs={"class": "form-check-input"}),
        }
    ...
    def clean_symbol(self):
        ...
        if not self._validate_symbol_with_api(symbol):
            raise ValidationError(
                f"Symbol '{symbol}' not found in Finnhub. Please check the symbol and try again."
            )
